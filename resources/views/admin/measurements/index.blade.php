...
<div class="card border-0 shadow-sm"><div class="card-body">
    ...
    <div class="table-responsive"><table class="table table-bordered align-middle"><thead><tr><th>Nama Anak</th><th>Gender</th><th>Tanggal</th><th>Usia <br>(Bulan)</th><th>Berat <br>(kg)</th><th>Status Berat</th><th>Tinggi <br>(cm)</th><th>Status Tinggi</th><th>Lingkar Kepala <br>(cm)</th><th>Status Kepala</th></tr></thead><tbody>
    @forelse($measurement as $row)
        <tr><td>{{ $row->child->name }}</td><td>{{ $row->child->gender === 'male' ? 'Laki-laki' : 'Perempuan' }}</td><td>{{ $row->measurement_date->format('d/m/Y') }} {{ substr($row->measurement_time,0,5) }}</td><td>{{ $row->child->ageMonths() }}</td><td>{{ $row->weight }}</td><td class="small">{{ $row->weight_status ?? '-' }}</td><td>{{ $row->height }}</td><td class="small">{{ $row->height_status ?? '-' }}</td><td>
            ...
            @else
                -
            @endif
        </td><td class="small">{{ $row->head_circumference_status ?? '-' }}</td></tr>
    @empty
